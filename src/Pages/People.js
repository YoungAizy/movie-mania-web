        ...
        <div>
            ...
            <form className='search-form'>
                <div className="search-container">
                    <input className='search-input' type="text" hint="Search for a person..." onChange={e => {setSearching(true); setSearchQuery(e.target.value)}}/>
                </div>
            </form>
...
export const CardPerson = ({ person, saveFave, isFav, setClicked, faved }) => {
                     ...
                     <div className="card-back" >
                        ...
                        <ul>
                            <li><strong>Department: </strong>{person.known_for_department}</li>
                                    <li><strong>Known For: </strong> { person['known_for'].map(movie=>(<i key={movie.id}>{movie.title}, </i>))}</li>
                            <li><strong>Popularity: </strong> 50.9</li>
                        </ul>
                        <div className='card-footer'>
                            <button className='btn-more' onClick={()=>getPersonInfo(person.id)}>More</button>
                            <IconButton onClick={e=>saveFave(e, person)} style={{justifySelf:'end', padding:"8px"}}><StarIcon style={{color:`${faved[person.id] ? "rgb(235, 222, 47)":"black"}`}}/></IconButton>
                        </div>
                    </div>
                    </div>
                </div>
